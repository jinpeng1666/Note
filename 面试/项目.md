...
- 先将需要秒杀的商品库存放入到 Redis 缓存中
- 利用 Lua 脚本（检查和扣减）进行库存的预先扣减，将下单请求封装为消息发送至消息队列
- 消息消费者进行消息的消费，在扣减 MySQL 库存时，使用悲观锁或者乐观锁，进行数据的扣减
...
- 说明：
    ...
    - JWT （JSON Web Token） 是目前最流行的跨域认证解决方案，是一种<font color="red">基于 Token 的认证授权机制</font>
...
- 定义：基于服务器会话的登录认证方法，是单体应用中最常见的登录状态维护方案
- 优点：
...
- 优点：
    - <font color="red">无状态</font>（是指服务器不需要存储用户的登录状态信息），天然适合分布式系统（不需要共享 Session）
...
- 常见问题与解决办法：
    - JWT 是自包含的，签发后在有效期内是不能被服务器主动废止的，导致注销、登录等场景下 JWT 还<font color="red">有效</font>的问题
...
- Gateway全局过滤器：具体来说，用户在前端登录成功之后，会拿到一个 JWT 令牌。后续所有请求都会带上这个令牌，首先会被 API 网关拦截。我们用的是 Spring Cloud Gateway，我在网关里写了一个全局过滤器，会统一解析 JWT，判断它是不是合法，比如签名、过期时间这些，如果不合法就直接返回 401。如果合法的话，我就把用户的 ID、角色这些信息提取出来，写进请求头里，然后把请求转发到后面的微服务。
- 拦截器：到了微服务这边，我做了一个拦截器，它会从请求头里把用户信息读出来，然后封装成一个用户对象，放进 ThreadLocal。这样的话，在业务逻辑里我们就可以很方便地拿到当前是谁在操作，比如做权限判断、数据过滤这些都能用上。请求结束之后我也有清除 ThreadLocal，防止内存泄漏。
- Feign拦截器：另外因为我们微服务之间有用到 Feign 做服务调用，所以我也实现了一个 Feign 的拦截器。它的作用就是在服务 A 调用服务 B 的时候，从 ThreadLocal 拿出当前用户的信息，然后再写到请求头里，这样服务 B 也能拿到用户信息，整个链路上的用户上下文就能一直保持。
