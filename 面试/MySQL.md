...
4. 调用<font color="red">数据库引擎接口</font>
    ...
    - 数据更新
        - 存储引擎根据执行器传入的数据，将目标数据页加载到 `Buffer Pool` 中（若不在内存）
        - 修改 `Buffer Pool`中的数据内容
        - 同时写入 `redo log buffer`，记录的是“对某个页的某个位置做了什么修改”
        - 根据刷盘时机，刷盘到 `redo log` 文件里（redo log 在事务执行过程中可以不断写入），状态为 `prepare`（预提交）
        
    - 记录 `binlog`（由 Server 层处理）
        - 存储引擎向执行器返回“已完成预提交”信号
        - 事务执行过程中，执行器将更新语句或其变更操作记录到 `binlog cache` 中（内存缓存）
        - 在事务提交时，再将 `binlog cache` 内容统一刷入 `bin log` 文件中，成为永久日志，用于主从复制和恢复
        
    - 提交 `redo log`（两阶段提交机制）
        - 如果 `bin log` 写入成功，执行器通知存储引擎将 `redo log` 状态从 `prepare` 更改为 `commit`
        - 此时事务正式提交完成，MySQL 采用 **WAL（Write-Ahead Logging）+ 两阶段提交（2PC）** 保证 binlog 与 redo log 的一致性
        
    - 更新完成
        - 执行器收到存储引擎的 commit 确认后，向客户端返回“更新成功”的响应
