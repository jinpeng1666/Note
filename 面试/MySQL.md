...
==索引（index）是帮助MySQL高效获取数据的数据结构(有序)==，主要用来提高数据检索效率，降低数据库的I/O成本同时，索引列可以对数据进行排序，降低数据排序的成本，也能减少CPU的消耗



## 1-2-InnoDB索引与MyISAM索引实现的区别是什么
...
相比于其他结构（如哈希表、B 树、红黑树等），选择B+树的原因包括：
...
1. **查询效率更稳定：**B树的非叶子节点和叶子节点都存放数据，而B+树的所有数据只出现在叶子节点，这使得B+树在查询时效率更稳定
2. **磁盘读写代价低：**相同数据量的情况下，B+树的高度更低，IO次数就越少，所以磁盘读写代价低
3. **范围扫描更高效：**B+树在进行范围查询时效率更高，因为所有数据都在叶子节点，并且叶子节点之间形成了双向链表



## 1-5-什么是聚集索引和非聚集索引

在InnoDB存储引擎中，根据==索引的存储形式==，又可以分为以下两种：
...
3. **联合索引：**尽量使用联合索引，减少单列索引，查询时，联合索引很多时候可以覆盖索引，节省存储空间，避免回表，提高查询效率
4. **索引数量：**要控制索引的数量，索引并不是多多益善，索引越多，维护索引结构的代价也就越大，会影响增删改的效率
5. **高区分度字段：**尽量选择区分度高的列作为索引，尽量建立唯一索引，区分度越高，使用索引的效率越高
6. **字符串字段：**如果是字符串类型的字段，字段的长度较长，可以针对于字段的特点，建立前缀索引
...
## 1-12-使用B+树存储的索引crud执行效率如何

使用 B+ 树作为索引结构，在执行 CRUD 操作时整体效率是比较高的

- 查询方面，它是有序结构，所有数据都在叶子节点上，通过从根节点查找到叶子节点，再加上叶子节点之间有链表连接，所以无论是精确查找还是范围查询，效率都非常高

- 插入和删除操作只影响局部的节点，可能涉及节点分裂或合并，但由于树高不大，这些操作通常不会带来太大开销。

- 更新的话，如果更新的是非索引字段，几乎不影响效率；但如果更新的是索引字段，相当于一次删除再插入，代价相对高一些。
...
- 顺序插入：新数据始终追加在 B+ 树最右端，避免页分裂
- 查询高效：结构紧凑、层级低，支持范围查找
- 节省空间：整型主键占用少，B+ 树扇出高、树更矮
- 缓存友好：热数据集中，提升缓存命中率

**⚠️ 字符串主键的劣势：**

- 插入无序：容易在中间插入，导致页分裂、结构碎片
- 查询开销大：多层跳转，可能降低性能
- 占用空间大：B+ 树扇出降低，树高增加
- 写入波动大：越多页分裂，写入越不稳定
...
- **聚簇索引（主键索引）**
    - 数据物理存储与索引顺序一致，支持顺序读写
    - 范围查询效率高，且天然有序返回
    - 对排序、分页、区间过滤等操作非常友好
- **非聚簇索引（二级索引）**
    - 条件查询时可避免全表扫描（如 `WHERE name = ?`）
    - 查询先通过索引定位主键，再回表查询完整数据
    - 支持范围、排序、分组等操作
    - 如果是覆盖索引（查询字段全部在索引中），则无需回表，效率更高
...
- 每个索引都对应一棵 独立的 B+ 树结构
- 每个 B+ 树由多个数据页组成，每个页通常为 16KB
- 索引越多，占用的存储空间越大，影响磁盘使用及缓存命中率

#### 写入/更新开销

- 增、删、改操作时，必须同步维护所有相关索引
- 索引更新可能引发：
    - 节点分裂（page split）
    - 节点合并（page merge）
    - 记录重排
    - 页面回收
- 当索引较多时，写操作成本显著上升，严重时影响整体性能
...
索引下推（Index Condition Pushdown，ICP）是MySQL 5.6引入的一种优化查询效率的技术，主要作用是：在存储引擎层提前使用 WHERE 子句的条件过滤数据，减少回表次数，提高查询效率



## 1-22-一个表有多个索引的时候，能否手动选择使用哪个索引
...
MySQL查询优化器会根据 **代价估算原则（Cost-Based Optimization）**，选择预计执行代价最低的那个索引
...
## 1-25-group by 分组和order by在索引使用上有什么区别
...
使用运维监控系统（Skywalking等）；如果没有这种监控系统，MySQL本身也提供了慢查询日志功能。可以在MySQL的系统配置文件中开启慢查询日志（默认不开启）（在MySQL的配置文件中配置），并设置SQL执行时间超过多少就记录到日志文件，比如我们之前项目设置的是2秒，超过这个时间的SQL就会记录在日志文件中，我们就可以在那里找到执行慢的SQL
...
- **字段类型选择合理**：根据实际数据选择合适的数值类型（如 `tinyint`, `int`, `bigint`），避免为字段分配不必要的大长度
- **字符串类型匹配**：

    - `CHAR`：适用于定长数据（如身份证号、手机号等），读取效率高
    - `VARCHAR`：适用于变长数据（如用户名、地址等），节省存储空间
- **拆分冗余字段**：遵循数据库设计范式（如第三范式），避免数据冗余和更新异常
- **添加必要约束**：如 `NOT NULL`、`DEFAULT`、`UNIQUE`、`CHECK` 等，有助于优化器做出更好判断
...
- 遵循索引创建原则：
	...
	- 常用于 `WHERE`、`ORDER BY`、`GROUP BY`、`JOIN` 的字段
	- 优先选择区分度高的字段
	- 使用联合索引覆盖查询
	- 控制索引数量，避免过多影响写性能

- 善用**覆盖索引**，避免回表操作，提升查询性能


- **考虑前缀索引**：对于长字符串字段，可以使用前缀索引节省空间



### 2-4-3-SQL语句优化

- **明确字段名称**：避免 `SELECT *`，只查询必要字段，减少 I/O 和网络传输压力
- **避免索引失效的写法**：
...
- **减少嵌套子查询**：能使用 JOIN 的场景不建议用子查询，子查询执行效率通常较低
...
MVCC通过维护数据的多个版本来避免读写冲突。底层实现包括隐藏字段、`undo log`和`read view`。隐藏字段包括`trx_id`和`roll_pointer`。`undo log`记录了不同版本的数据，通过`roll_pointer`形成版本链。`read view`包含了四个核心字段，定义了不同隔离级别下的快照读，决定了事务访问哪个版本的数据。
...
- `DB_TRX_ID` 用来标识最近一次对本行记录做修改 (insert 、update) 的事务的标识符 ，即最后一次修改本行记录的事务 id。 如果是 delete 操作， 在 InnoDB 存储引擎内部也属于一次 update 操作，即更新行中的一个特殊位 ，将行标识为己删除，并非真正删除
...
2. SQL必须完全一致才会导致**cache命中**
3. 为了节省**内存空间**，太大的result set不会被cache (< query_cache_limit)
4. MySQL缓存在**分库分表**环境下是不起作用的
5. 执行SQL里有触发器,自定义函数时，MySQL缓存也是不起作用的
6. 在表的结构或数据发生改变时，基于该表相关cache立即全部**失效**
